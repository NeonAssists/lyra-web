...
function RatingBadge({ rating }: { rating: number }) {
  return (
    <span className="text-sm font-black" style={{ color: ratingColor(rating) }}>
      {rating.toFixed(1)}
    </span>
  );
}
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] pb-24 md:pb-8">
      ...
      <nav className="sticky top-0 z-40 bg-[#0a0a0a]/90 backdrop-blur-xl border-b border-white/[0.06] px-4 py-3 flex items-center gap-3">
        ...
        <Link href="/u/nate7" className="w-8 h-8 rounded-full bg-[#6C63FF] flex items-center justify-center text-xs font-bold flex-shrink-0">N</Link>
      </nav>
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-3">
                  {topSongs.slice(0, 10).map((song, i) => (
                    <div key={song.id} className="bg-[#141414] border border-white/[0.06] rounded-2xl overflow-hidden hover:border-[#6C63FF]/40 transition-all hover:scale-[1.02] cursor-pointer group">
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-3">
                  {newAlbums.slice(0, 10).map((album) => (
                    <div key={album.id} className="bg-[#141414] border border-white/[0.06] rounded-2xl overflow-hidden hover:border-[#6C63FF]/40 transition-all hover:scale-[1.02] cursor-pointer">
      ...
      <div className="fixed bottom-0 left-0 right-0 md:hidden bg-[#0a0a0a]/90 backdrop-blur-xl border-t border-white/[0.06] flex items-center justify-around px-6 py-3">
        ...
        <Link href="/u/nate7" className="flex flex-col items-center gap-1 text-[#8E8E93]">
