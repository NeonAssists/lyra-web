...
export default function AppHome() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // World Music Week — curated tracks from schedule
              ...
              <Box>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px 0' }}>
                  <div>
                    ...
                    <h2 style={{ fontSize: 16, fontWeight: 800, color: '#fff', letterSpacing: '-0.2px', margin: 0 }}>{worldWeek.flag} {worldWeek.region}</h2>
                  </div>
                ...
                <p style={{ padding: '6px 20px 12px', fontSize: 13, color: 'rgba(255,255,255,0.4)', lineHeight: 1.5 }}>
                  {worldWeek.description}
                </p>
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.05)', padding: '8px 0' }}>
                  {worldSongs.slice(0, 10).map((s: any, i: number) => (
                    <ListRow key={`wms-${i}-${s.id}`} item={{ title: s.title, artist: s.artist, artwork_url: s.artwork, rating: 0 }} rank={i + 1}
